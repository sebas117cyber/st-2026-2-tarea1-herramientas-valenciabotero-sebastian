...
leer_serie <- function(x, fuente = "desconocida", unidad = "sin unidad") {
  ...
  if (inherits(x, "ts")) {
    ...
    val_y <- as.numeric(x)
    n       <- length(val_y)    # numero de datos 
 ...
 # VERIFICAR QUE LAS FECHAS SEAN CORRECTAS
  ...
  # construccion de tsible
  
  resultado <- tibble(
    t     = seq_len(n),   # indice entero desde 1
    fecha = fechas,       # clase Date para las fechas
    valores     = val_y       # valores numericos
  )
